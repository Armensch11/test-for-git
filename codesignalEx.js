...
function chessboardDraw(n, k) {
	...
	for (let i = 8; i >= 1; i--) {
		...
		chessArr.push(temprow);
	}
	return chessArr;
}

//aveli parz shaxmati dasht
function chessboardDrawNew(n, k) {
	let chessArr = [];
	for (let i = n; i >= 1; i--) {
		let temprow = [];
		for (let j = 1; j <= k; j++) {
...
//let board = chessboardDrawNew(8, 8);
//console.log(knightPositionAndInfluence(1, 1, board));

//taguhu harvatsnery 8*8 dashti vra
function queenPositionAndInfluence(a, b, board) {
	a = 8 - a;
	b = b - 1;
	for (let i = board.length - 1; i >= 0; i--) {
		for (let j = 0; j < board[0].length; j++) {
			if (board[i]) {
				board[a][j] = 'q';
				board[i][b] = 'q';
				if ((i === a - (a - i) && j === b - (a - i)) || (i === a + (i - a) && j === b - (i - a))) {
					board[i][j] = 'q';
					step++;
				}
			}
...
//dziu qajlery 8*8 dashti vra
function knightPositionAndInfluence(a, b, board) {
	a = 8 - a;
	b = b - 1;
	for (let i = board.length - 1; i >= 0; i--) {
		for (let j = 0; j < board[0].length; j++) {
...
//bishop and pawn exercise
function bishopAndPawn(bishop, pawn) {
	let letter = [ 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h' ];
	let a = bishop[1] - 1;
	let b = letter.indexOf(bishop[0]);
	let iPawn = pawn[1] - 1;
	let jPawn = letter.indexOf(pawn[0]);
	for (let i = 8 - 1; i >= 0; i--) {
		for (let j = 0; j < 8; j++) {
			if ((iPawn === a - (a - i) && jPawn === b - (a - i)) || (iPawn === a + (i - a) && jPawn === b - (i - a))) {
				return true;
			}
		}
	}
...
function stringsRearrangement(inputArray) {
	let countArr = [ inputArray[0] ];

	for (let i = 0; i < inputArray.length; i++) {
		for (let j = 0; j < inputArray.length; j++) {
			if (i !== j) {
				if (compareStrforOneDiff(inputArray[i], inputArray[j])) {
					countArr.push(inputArray[j]);
				}
			}
		}
		if (countArr.length === inputArray.length) {
			return true;
		}
	}
	return false;
...
//console.log(stringsRearrangement([ 'zzzzab', 'zzzzbb', 'zzzzaa' ]));

function compareStrforOneDiff(mainStr, checkStr) {
	let count = 0;
	for (let i = 0; i < mainStr.length; i++) {
		if (checkStr.indexOf(mainStr[i]) >= 0) {
			count++;
		}
	}
	if (count === mainStr.length - 1) {
		return true;
	} else {
		return false;
	}
